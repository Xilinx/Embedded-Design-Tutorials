...
### U-Boot
 ...
 PetaLinux tool flow. For this example, you can use the U-Boot image
 that you created in [Chapter 5: System Design Example](#chapter-5)
 [using Scalar Engine and Adaptable Engine](#chapter-5).
...
2. Copy the sd_boot.bif file present within the
...
###### Related Information
 ...
 [Locating Design Files](#locating-design-files)
...
1. To verify the image:

    - For standalone, copy the `BOOT.BIN` to the SD card.

    - For Linux images, navigate to the `<petalinux-project>/images/linux` and copy `BOOT.BIN`, `image.ub`, and `boot.scr` to the SD card.
