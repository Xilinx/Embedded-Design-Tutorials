...
This section will start with the VCK190 or VMK180 project built in the prior chapter (or the pre-built project in the design package `<design-package>/smartlynq_plus/vck190/design_files/vck190_edt_versal_hsdp.xpr.zip`). We will modify the project to include HSDP support.
...
1.  Change the working directory to the PetaLinux build root if working on the machine used to build PetaLinux, or the location where the `images/Linux` directory was transferred to the local machine in the prior steps.
