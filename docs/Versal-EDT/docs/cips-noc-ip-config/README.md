- [Versal ACAP CIPS and NoC (DDR) IP Core Configuration](#versal-acap-cips-and-noc-ddr-ip-core-configuration)
    - [Prerequisites](#prerequisites)
  - [CIPS IP Core Configuration](#cips-ip-core-configuration)
    - [Example Project: Creating a New Embedded Project with the Versal ACAP](#example-project-creating-a-new-embedded-project-with-the-versal-acap)
      - [Starting Your Design](#starting-your-design)
      - [Creating an Embedded Processor Project](#creating-an-embedded-processor-project)
      - [Managing the Versal ACAP CIPS IP Core in the Vivado Design Suite](#managing-the-versal-acap-cips-ip-core-in-the-vivado-design-suite)
      - [OSPI Boot Mode Configuration](#ospi-boot-mode-configuration)
    ...
    - [Example Project: Bare-metal Hello World Project with APU on OCM and RPU on TCM](#example-project-bare-metal-hello-world-project-with-apu-on-ocm-and-rpu-on-tcm)
    - [Creating a Hello World Application for the Arm Cortex-A72 on OCM](#creating-a-hello-world-application-for-the-arm-cortex-a72-on-ocm)
      - [Modifying the helloworld_a72 Application Source Code](#modifying-the-helloworld_a72-application-source-code)
    - [Adding a New RPU Domain to the Platform Project](#adding-a-new-rpu-domain-to-the-platform-project)
    - [Creating the Standalone Application Project for the Arm Cortex-R5F](#creating-the-standalone-application-project-for-the-arm-cortex-r5f)
        - [Modifying the helloworld_r5 Application Source Code](#modifying-the-helloworld_r5-application-source-code)
        - [Modifying the Application Linker Script for the Application Project helloworld_r5](#modifying-the-application-linker-script-for-the-application-project-helloworld_r5)
    - [Running Applications in the JTAG Mode using the System Debugger in the Vitis Software Platform](#running-applications-in-the-jtag-mode-using-the-system-debugger-in-the-vitis-software-platform)
      - [Creating a Run Configuration for the System Project](#creating-a-run-configuration-for-the-system-project)
      - [Creating a Run Configuration for a Single Application within a System Project](#creating-a-run-configuration-for-a-single-application-within-a-system-project)
        - [Method I](#method-i)
        - [Method II](#method-ii)
  - [NoC (and DDR) IP Core Configuration](#noc-and-ddr-ip-core-configuration)
    - [Example Project: Configuring the NoC IP Core in an Existing Project](#example-project-configuring-the-noc-ip-core-in-an-existing-project)
      - [Configuring Your Design](#configuring-your-design)
      - [Validating the Design and Generating the Output](#validating-the-design-and-generating-the-output-1)
      - [Synthesizing, Implementing, and Generating the Device Image](#synthesizing-implementing-and-generating-the-device-image-1)
      - [Exporting Hardware](#exporting-hardware-1)
    - [Example Project: Creating a bare-metal Hello World System Project with APU and RPU Running on DDR Memory](#example-project-creating-a-bare-metal-hello-world-system-project-with-apu-and-rpu-running-on-ddr-memory)


# Versal ACAP CIPS and NoC (DDR) IP Core Configuration

This chapter describes how to create a Vivado® project for the Versal™
 ACAP by configuring the CIPS (Control, Interfaces, and Processing
 System) IP core and the NoC IP core. It also demonstrates how to
 create and run standalone hello world applications on APU and RPU. The
 Versal ACAP CIPS IP core allows you to configure the processing system
 and the PMC block including boot mode, peripherals, clocks,
 interfaces, and interrupts, among other things. The NoC IP core helps
 configure DDR and data path across DDR and processing engines in the
 system (Scalar Engines, Adaptable Engines, and AI Engines).
...
### Example Project: Creating a New Embedded Project with the Versal ACAP
...
3. Use the following information in the table to make selections in
...
>**NOTE:**  Check the version number while choosing a boards. For ES1
...
5. Expand the **PS-PMC** drop-down menu. Click **IO Configuration** as
    ...
    allows selecting pins assignments for the associated MIO. The Run
    block Automation configures the peripherals as shown in the following
    figure:
...
#### OSPI Boot Mode Configuration

 The following section provides the steps to configure the design for
 the OSPI boot mode. You can either use QSPI or OSPI as the boot mode.
 Both modes cannot be used simultaneously. If you do not want to use
 the OSPI boot mode, you can skip this section and go to [Validating
 the](#validating-the-design-and-generating-the-output) [Design and
 Generating the
 Output](#validating-the-design-and-generating-the-output).

1. Double-click **versal_cips_0** in the Block Diagram window.

2. In the Configuration Options pane, expand **Boot Mode** and select
     **IO configuration**.

    ![](media/image11.jpeg)

   By default, the board preset enables the QSPI boot mode.

3. Uncheck the QSPI checkbox under Boot Peripherals and check the
     **OSPI** checkbox.

    ![](media/image12.png)

4. Click **PS-PMC → IO Configuration**.

    The MIO configuration for OSPI is as shown in the figure.

    ![ospi_mio](media/OSPI_mio.PNG)

5. Click **OK** to close the CIPS GUI. This sets the boot mode as OSPI.
...
### Example Project: Bare-metal Hello World Project with APU on OCM and RPU on TCM
...
4. Launch the Vitis software platform and set the workspace path, which
    in this example is `C:\\edt\\edt_vck190`.
...
6. Click **Connect to a serial port** button ![](media/image22.png) in the Vitis terminal context to
     set the serial configuration and connect it.
...
2. Click the ![](./media/image30.png){width="0.26045166229221345in"
     height="0.22919728783902013in"} button .
...
### Example Project: Configuring the NoC IP Core in an Existing Project
...
### Example Project: Creating a bare-metal Hello World System Project with APU and RPU Running on DDR Memory
