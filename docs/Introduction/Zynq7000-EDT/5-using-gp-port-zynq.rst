...
The flow of this chapter is similar to that in `Using the Zynq SoC Processing System <2-using-zynq.rst>`__ and uses the Zynq device as a base hardware design. It is assumed that you understand the concepts discussed in `Using the Zynq SoC Processing System <./2-using-zynq.rst>`__ regarding adding the Zynq device into a Vivado IP integrator block diagram design.
...
You can use the system created in `Using the Zynq SoC Processing System <2-using-zynq.rst>`__ and continue with the following examples.
...
-  Input Files:
   ...
   -  Vivado project from `Example 1 <./2-using-zynq.rst#example-1-creating-a-new-embedded-project-with-zynq-soc>`__
   -  Vitis workspace in `Example 2 <./2-using-zynq.rst#example-2-creating-and-running-a-hello-world-application>`__
...
1. Open the Vivado design created in `Example 1 <./2-using-zynq.rst#example-1-creating-a-new-embedded-project-with-zynq-soc>`__:

   1. Launch the Vivado |reg| IDE.
   2. Under the Recent Projects column, click the **edt_zc702** design that you created in `Using the Zynq SoC Processing System <./2-using-zynq.rst#example-1-creating-a-new-embedded-project-with-zynq-soc>`__.
...
5. Find **gpio_sw_tri_io[0]** and set the following properties, shown in the following figure:
   ...
   .. note:: For additional information about creating other design constraints, refer to the *Vivado Design Suite User Guide: Using Constraints* (`UG903 <https://www.xilinx.com/cgi-bin/docs/rdoc?v=2021.1;d=ug903-vivado-using-constraints.pdf>`__).
...
7. Change boot mode back to JTAG mode (as in `Example 2 <./2-using-zynq.rst#setting-up-the-board>`__).

   - Set **SW16** to 00000.

8. Run the project similar to the steps in `Example 2 <./2-using-zynq.rst#running-the-hello-world-application-on-a-zc702-board>`__.
...
See the `next chapter <./6-using-hp-port.rst>`__ for information about using the AXI HP (High Performance) slave port with the AXI Central DMA IP.
