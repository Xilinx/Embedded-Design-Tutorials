...
-  Input files:
   ...
   -  Vivado design from `Example 6 <5-using-gp-port-zynq.rst#example-6-adding-peripheral-pl-ip>`__
   -  Vitis workspace
...
1. Start with the system you created in `Example 6: Adding Peripheral PL IP <5-using-gp-port-zynq.rst#example-6-adding-peripheral-pl-ip>`__.

   1. Open the Vivado |reg| design from `Example 6 <5-using-gp-port-zynq.rst#example-6-adding-peripheral-pl-ip>`__.
...
5. In the Import Sources dialog box, click the **Browse** button next to the **From directory** field and specify the design files folder to `ref_files/example7 <https://github.com/Xilinx/Embedded-Design-Tutorials/tree/master/docs/Introduction/Zynq7000-EDT/ref_files/example7>`__.
...
2. Make sure that the boot mode of the board is set to JTAG and power on.
   ...
   .. note:: Refer to `Board Setup in Example 2 <2-using-zynq.rst>`__ for information about setting up the board.
