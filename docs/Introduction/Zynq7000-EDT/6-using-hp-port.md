...
Till now, our examples are all developed and debugged in a lab environment. It requires JTAG to configure and control the applications. How can we deploy applications to the board so that it can run the applications after boot by itself? See the [next chapter](./7-linux-booting-debug.md) for Linux boot image configuration.
