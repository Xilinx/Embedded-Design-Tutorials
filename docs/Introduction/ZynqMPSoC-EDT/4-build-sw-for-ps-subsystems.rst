...
In the previous chapter, `Zynq UltraScale+ MPSoC Processing System Configuration <3-system-configuration.rst>`__, you created and exported
...
1. Launch the Vitis IDE:
   ...
   -  From Windows Start menu, select **Xilinx Design Tools → Xilinx Vitis 2021.1**; or
   -  Double-click the ``C:\Xilinx\Vitis\2021.1\bin\vitis.bat`` file.
...
4. Click **Run**.
   ...
   More debugging techniques are explored in the `next chapter <./5-debugging-with-vitis-debugger.rst>`__.
...
-  Input:
   ...
   -  Source code for Arm Cortex-R5F:
      `ref_files/example5/testapp_r5.c <https://github.com/Xilinx/Embedded-Design-Tutorials/tree/master/docs/Introduction/ZynqMPSoC-EDT/ref_files/example5>`__
...
Use the same steps as `Example 3 <#example-3-running-the-hello-world-application-from-arm-cortex-a53>`__, but this time create the system project with name ``hello_system``.
...
2. Import the prepared source code for **testapp_r5**:
   ...
   3. In the **From directory** field, select **Browse** and navigate to the design files folder (`ref_files/example5 <./ref_files/example5>`__).
   4. Click **OK**.
...
2. Connect the serial console for UART-0 and UART-1:
   ...
   1. Use the `MobaXterm <https://mobaxterm.mobatek.net/>`__ utility to connect multiple UART ports.
...
In the `next chapter <./5-debugging-with-vitis-debugger.rst>`__, you will learn about debugging standalone applications with the Vitis Debugger.
