...
-  Input:
   ...
   -  `ZCU102 PetaLinux
      BSP <https://www.xilinx.com/support/download/index.html/content/xilinx/en/downloadNav/embedded-design-tools.html>`__
...
.. important::
   ...
   2. This example uses the `ZCU102 PetaLinux BSP <https://www.xilinx.com/support/download/index.html/content/xilinx/en/downloadNav/embedded-design-tools.html>`__ to create a PetaLinux project. Ensure that you have downloaded the ZCU102 BSP for PetaLinux as instructed on the `PetaLinux Tools download page <https://www.xilinx.com/member/forms/download/xef.html?filename=xilinx-zcu102-v2021.1-final.bsp>`__.
...
1. Create a PetaLinux project using the following command:
   ...
   .. note:: ``xilinx-zcu102-v2021.1-final.bsp`` is the PetaLinux BSP for the ZCU102 Production Silicon Rev 1.0 Board.

   This creates a PetaLinux project directory, ``xilinx-zcu102-2021.1``.
...
2. Reconfigure the project with `edt_zcu102_wrapper.xsa`:
   ...
   -  The created PetaLinux project uses the default hardware setup in the ZCU102 Linux BSP. In this example, you will reconfigure the
      PetaLinux project based on the Zynq UltraScale+ hardware platform that you configured using the Vivado |reg| Design Suite in `Zynq
      UltraScale+ MPSoC Processing System Configuration <3-system-configuration.rst>`__.
...
3. Build the PetaLinux project:
   ...
   1. In the ``<PetaLinux-project>`` directory, for example, ``xilinx-zcu102-2021.1``, build the Linux images using the
      following command:
...
4. Generate the boot image using the following command:
   ...
   The logs indicate that the above command includes PMU_FW and ATF in ``BOOT.BIN``. You can also add ``--pmufw <PMUFW_ELF>`` and
...
1. Prepare for running the Linux application on the ZCU102 board. Vitis can download the Linux application to the board, which runs Linux through a network connection. It is important to ensure that the connection between the host machine and the board works well.
   ...
   3. Power on the board and let Linux run on ZCU102 (see `Verifying the Image on the ZCU102 Board <#verifying-the-image-on-the-zcu102-board>`__).
...
In the `next chapter <./7-design1-using-gpio-timer-interrupts.rst>`__, you will connect all points previously introduced and create a system design.
