...
.. note:: If you did not create a “Hello World” application on the APU or RPU, follow the steps in `Create Custom Bare-Metal Application for Arm Cortex-A53 based APU <4-build-sw-for-ps-subsystems.rst#creating-a-custom-bare-metal-application-for-an-arm-cortex-a53-based-apu>`__ to create a new “Hello World” application.
...
1. Connect the JTAG cable, set the boot mode to JTAG, and power on the board. Refer to the steps in `Example 3: Running the “Hello World”
   Application from Arm Cortex-A53 <4-build-sw-for-ps-subsystems.rst#example-3-running-the-hello-world-application-from-arm-cortex-a53>`__.
...
6. You can now run the code:
   ...
   The `next chapter <./6-build-linux-sw-for-ps.rst>`__ shows how to build and debug Linux applications.
