...
This chapter describes the steps required to create an embedded design in the AMD Vivado Design Suite using the Versal Embedded Common Platform – Simple PL Example design for the VPK180 Evaluation Platform.

The VPK180 configurable example design targets the AMD Versal Premium VP1802 Adaptive SoC and provides a reference hardware platform containing the Versal CIPS subsystem, NoC infrastructure, DDR memory connectivity, PL resources, and debug components. The design demonstrates the integration of processing system resources with programmable logic and NoC based memory connectivity, enabling embedded software execution on the Arm Cortex-A72 Application Processing Unit (APU) and Arm Cortex-R5F Real-time Processing Unit (RPU).

The hardware platform generated in this chapter is exported as an AMD Vivado hardware handoff file (XSA) and used in the AMD Vitis Unified Software Platform for software application development. The platform can serve as the foundation for standalone applications, Linux-based systems, and accelerated embedded designs.

This chapter also describes how to configure, build, and deploy a Linux operating system targeting the Arm Cortex-A72 processors on the VPK180 platform. The flow covers hardware platform generation, software platform creation, Linux image generation, and system boot.

For more information on designing with Stacked Silicon Interconnect (SSI) devices, refer to the *UltraFast™ Design Methodology Guide for FPGAs and SoCs* `(UG949) <https://docs.amd.com/r/en-US/ug949-vivado-design-methodology/Designing-with-SSI-Devices>`__.
...
This design example uses a PL-based AXI GPIO interface to control the LEDs on the board using a Linux application. 

The RPU bare-metal example application uses a PL-based AXI UART lite to print debug messages to the AXI UART console instead of the PS UART console.

To create a project for the VPK180 Evaluation Platform, refer to the :ref:`Creating a New Embedded Project with a Versal Device <.. _5-creating-embedded-project:>`.

After Step 6 in the Creating a New Embedded Project with a Versal Device section, when creating a project for SSI devices, select **VPK180 Evaluation Platform** as the Display Name.

After the project is created, the Vivado Design Suite automatically opens the example block design as shown in the following figure:

   .. image:: ./media/ch7_block_design.png
...
1. In the Block Design, locate the ``ilconstant_0`` block connected to the ``m_axi_lpd_aclk`` pin of the ``CIPS_0`` block.
   ...
   .. image:: ./media/ch7_ilconstant_0.png
...
The overall block design is shown in the following figure:
   ...
   .. image:: ./media/ch7_block_design_2.png
...
2.	The tool verifies the design connections, clocking, and configuration.
   ...
   Once the validation is complete, A message dialog box opens:
      ...
      .. image:: ./media/ch7_validation_successful.png
...
5. Right-click the top-level block design, ``edf_base_pl_i : edf_base_pl`` (``edf_base_pl.bd``), and select **Generate Output Products**.
   ...
   .. image:: ./media/ch7_generate_output_products.png
...
8.	In the Sources window, click the **IP Sources** view. Here, you can see the output products that you just generated, as shown in the following figure.
   ...
   .. image:: ./media/ch7_output_products.png
...
2.	A No Implementation Results Available menu appears. Click **Yes**.
   ...
   .. image:: ./media/ch7_no_implementation_results_available.png

3. A Launch Runs menu appears. Click **OK**.

   .. image:: ./media/ch7_launch_runs.png

4. When the Device Image Generation is completed, the Device Image Generation Completed dialog box opens.

   .. image:: ./media/ch7_device_image_generation_completed.png
...
..
      .. note:: The following steps are optional, you can skip these and go to the :ref:`exporting-hardware-7` section.
      ...
      1.	Go to **Flow Navigator → Synthesis** and click **Run Synthesis**.
         ...
         .. image:: ./media/ch7_run_Synthesis.png
