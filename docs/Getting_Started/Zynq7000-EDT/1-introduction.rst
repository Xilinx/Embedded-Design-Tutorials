...
The AMD Zynq |trade| 7000 SoC comes with a versatile processing system (PS) integrated with a highly flexible and high-performance programmable logic (PL) section, all on a single system-on-a-chip (SoC).

The PS and the PL in AMD Zynq |trade| UltraScale+ devices can be tightly or loosely coupled through various high-performance and high-bandwidth PS-PL interfaces.

How AMD Software Simplifies Embedded Processor Designs
---------------------------------------------------------
...
The Zynq |trade| SoC solution reduces this complexity by offering an Arm |reg| Cortex |trade|-A9 dual core, along with programmable logic, all within a single SoC. To simplify the design process, AMD offers the Vitis |trade| software platform. This set of tools provides you with everything you need to simplify embedded system design for a device that integrates an SoC with an FPGA. This combination of tools offers hardware and software application design, debugging capability, code execution, and transfer of the design onto actual boards for verification and validation.

Vitis Unified Software Platform
-------------------------------

The Vitis software platform includes all the tools that you need to develop, debug, and deploy your embedded applications.

It includes the AMD Vivado |trade| Design Suite, which can create hardware designs for the SoC. The hardware design includes the PL logic design, the configuration of PS, and the connection between PS and PL.
...
The Vitis installer also includes PetaLinux. PetaLinux helps build the embedded Linux environment for AMD SoC.

Vivado Design Suite
~~~~~~~~~~~~~~~~~~~

The Vivado Design Suite provides full features of AMD FPGA and SoC hardware design, including code editing, synthesis, implementation, simulation, and binary generation. It also provides PS configuration and initialization code generation features.
...
The major differences between editions lie in the supported `device architectures <https://www.xilinx.com/products/design-tools/vivado/vivado-ml.html#architecture>`_.
...
Use the Vivado Design Suite tools to add design sources to your hardware. These include the IP integrator, which simplifies the process of adding IP to your existing project and creating connections for ports (such as clock and reset).
...
The Vitis software platform is used for software development, and can be installed and used without any other AMD tools installed on the machine it is loaded.  
...
Visit the `Xilinx Download Center <https://www.xilinx.com/support/download.html>`_ to download the Vitis software platform. This tutorial is verified with 2023.2. If you are using other Vitis versions, some features or screenshots might differ.

The Vitis software platform supports Windows and Linux. To install the Vitis software platform, follow the instructions provided in the `Installation section <https://docs.xilinx.com/r/en-US/ug1400-vitis-embedded/Getting-Started-with-Vitis>`_ of the *Vitis Unified Software Platform Documentation: Embedded Software Development* (`UG1400 <https://docs.xilinx.com/r/en-US/ug1400-vitis-embedded/Getting-Started-with-Vitis>`_). When you install the Vitis software platform, the Vivado Design Suite is installed automatically.

Use Vivado to create the hardware design and Vitis to develop both Linux and standalone software applications.
...
Start with the first examples in the next chapter.
