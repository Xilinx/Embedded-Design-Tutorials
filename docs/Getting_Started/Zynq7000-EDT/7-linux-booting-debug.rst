...
1. Use the below command to launch your project configuration settings
...
3. Change partition 0 to `qspi-boot` and set the size to `0xF40000` and change partiton 1 to `qspi-kernel` and set the size to `0x900000`. If there are any other partions then delte them.

   .. image:: ./media/flash_settings.png

   Save the settings and exit configuration window.
