...
**Note** When using the FSBL to configure the ps7 susbsystem, the debugger will set a breakpoint on the exit function of the FSBL. The debugger will wait until this breakpoint is hit, or a timeout occurs. If the timeout occurs, then this mean the breakpoint was not hit. This could be due to the symbol info not found. This could happen if users are connecting remotely and a **sysmbol server** in the Target Connections can be used.
