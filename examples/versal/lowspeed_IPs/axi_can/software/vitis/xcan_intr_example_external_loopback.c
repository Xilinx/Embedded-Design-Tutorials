 ...
 */
#define CAN_DEVICE_ID_0		XPAR_CAN_0_DEVICE_ID
#define CAN_DEVICE_ID_1		XPAR_CAN_1_DEVICE_ID
#define CAN_DEVICE_ID_2		XPAR_CAN_2_DEVICE_ID
#define CAN_INTR_VEC_ID_1		XPAR_FABRIC_CAN_1_VEC_ID
#define CAN_INTR_VEC_ID_2		XPAR_FABRIC_CAN_2_VEC_ID
...
static int SetupInterruptSystem(XCan *InstancePtr, int id);
...
int main()
{
	 ...
	 */
	if (XCanIntrExample(CAN_DEVICE_ID_1)) {
		xil_printf("Can Interrupt Example Failed\r\n");
		return XST_FAILURE;
	}

	xil_printf("Successfully ran Can Interrupt Example with external loopback\r\n");
	return XST_SUCCESS;
}
...
static int XCanIntrExample(u16 DeviceId)
{
	 ...
	 */
	Status = XCan_Initialize(&Can_1, CAN_DEVICE_ID_1);   // Initialize AXI CAN 1
	if (Status != XST_SUCCESS) {
		return XST_FAILURE;
	}

	Status = XCan_Initialize(&Can_2, CAN_DEVICE_ID_2);	// Initialize AXI CAN 2
		if (Status != XST_SUCCESS) {
			return XST_FAILURE;
		}
	 ...
	 */
	XCan_SetHandler(&Can_1, XCAN_HANDLER_SEND,
			(void *)SendHandler, (void *)&Can_1);
	XCan_SetHandler(&Can_1, XCAN_HANDLER_RECV,
			(void *)RecvHandler, (void *)&Can_1);
	XCan_SetHandler(&Can_1, XCAN_HANDLER_ERROR,
			(void *)ErrorHandler, (void *)&Can_1);
	XCan_SetHandler(&Can_1, XCAN_HANDLER_EVENT,
			(void *)EventHandler, (void *)&Can_1);

	XCan_SetHandler(&Can_2, XCAN_HANDLER_SEND,
			(void *)SendHandler, (void *)&Can_2);
	XCan_SetHandler(&Can_2, XCAN_HANDLER_RECV,
			(void *)RecvHandler, (void *)&Can_2);
	XCan_SetHandler(&Can_2, XCAN_HANDLER_ERROR,
			(void *)ErrorHandler, (void *)&Can_2);
	XCan_SetHandler(&Can_2, XCAN_HANDLER_EVENT,
			(void *)EventHandler, (void *)&Can_2);
	/*
	 * Initialize flags.
	 */
	SendDone = FALSE;
	 ...
	 */
	Status = SetupInterruptSystem(&Can_1, CAN_INTR_VEC_ID_1);
	if (Status != XST_SUCCESS) {
		return XST_FAILURE;
	}

	Status = SetupInterruptSystem(&Can_2, CAN_INTR_VEC_ID_2);
		if (Status != XST_SUCCESS) {
			return XST_FAILURE;
		}
	 ...
	 */
	XCan_InterruptEnable(&Can_1, XCAN_IXR_ALL);
	XCan_InterruptEnable(&Can_2, XCAN_IXR_ALL);
	/*
	 * Enter Loop Back Mode.
	 */
	XCan_EnterMode(&Can_1, XCAN_MODE_NORMAL);
	XCan_EnterMode(&Can_2, XCAN_MODE_NORMAL);
	while(XCan_GetMode(&Can_1) != XCAN_MODE_NORMAL);
	while(XCan_GetMode(&Can_2) != XCAN_MODE_NORMAL);

	/*
	 * Loop back a frame. The RecvHandler is expected to handle
	 * the frame reception.
	 */
	SendFrame(&Can_1); /* Send a frame */

	/*
	 * Wait here until both sending and reception have been completed.
	 */
	while ((SendDone != TRUE) || (RecvDone != TRUE));
	XCan_Recv(&Can_2, RxFrame);

	for(int data = 0; data < 4; data++)
	{
		if(TxFrame[data] != RxFrame[data])
			return XST_FAILURE;
	}
	/*
...
*
****************************************************************************/
static int SetupInterruptSystem(XCan *InstancePtr, int int_id)
{
