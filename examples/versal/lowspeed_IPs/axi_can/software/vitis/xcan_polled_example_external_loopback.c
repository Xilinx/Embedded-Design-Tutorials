...
int main(void)
{
	...
	xil_printf("Successfully ran Can polled Example with external loop back\r\n");
	return XST_SUCCESS;
...
int XCanPolledExample(u16 DeviceId)
{
	 ...
	 */
	Status = XCan_Initialize(&Can_1, 1);
	if (Status != XST_SUCCESS) {
		return Status;
	}

	Status = XCan_Initialize(&Can_2, 2);
		if (Status != XST_SUCCESS) {
			return Status;
		}
	 ...
	 */
	XCan_EnterMode(&Can_1, XCAN_MODE_CONFIG);
	while(XCan_GetMode(&Can_1) != XCAN_MODE_CONFIG);
	XCan_EnterMode(&Can_2, XCAN_MODE_CONFIG);
	while(XCan_GetMode(&Can_2) != XCAN_MODE_CONFIG);
	 ...
	 */
	XCan_SetBaudRatePrescaler(&Can_1, TEST_BRPR_BAUD_PRESCALAR);
	XCan_SetBitTiming(&Can_1, TEST_BTR_SYNCJUMPWIDTH,
				TEST_BTR_SECOND_TIMESEGMENT,
				TEST_BTR_FIRST_TIMESEGMENT);
	XCan_SetBaudRatePrescaler(&Can_2, TEST_BRPR_BAUD_PRESCALAR);
		XCan_SetBitTiming(&Can_2, TEST_BTR_SYNCJUMPWIDTH,
					TEST_BTR_SECOND_TIMESEGMENT,
					TEST_BTR_FIRST_TIMESEGMENT);
	/*
	 ...
	 * contents.
	 */
	Status = SendFrame(&Can_1);
	if (Status != XST_SUCCESS) {
		return Status;
	}

	Status = RecvFrame(&Can_2);
