 ...
 */
#define CANFD_DEVICE_ID	XPAR_CANFD_1_DEVICE_ID
...
int main(void)
{
	...
	xil_printf("Successfully ran AXI-XCanFd Polled Mode example with external loopback\n\r");
	return XST_SUCCESS;
...
{
	int Status;
	XCanFd *CanFdInstPtr_1 = &CanFd_1;
	XCanFd *CanFdInstPtr_2 = &CanFd_2;
	...
	Status = XCanFd_CfgInitialize(CanFdInstPtr_1,
					ConfigPtr_1,
					ConfigPtr_1->BaseAddress);
	if (Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
	Status = XCanFd_CfgInitialize(CanFdInstPtr_2,
						ConfigPtr_2,
						ConfigPtr_2->BaseAddress);
		if (Status != XST_SUCCESS) {
			return XST_FAILURE;
		}
	/*
	 ...
	 */
	Status = XCanFd_SelfTest(CanFdInstPtr_1);
	if (Status != XST_SUCCESS) {
		return Status;
	}

	Status = XCanFd_SelfTest(CanFdInstPtr_2);
		if (Status != XST_SUCCESS) {
			return Status;
		}
	/*
	 ...
	 */
	XCanFd_EnterMode(CanFdInstPtr_1, XCANFD_MODE_CONFIG);
	while (XCanFd_GetMode(CanFdInstPtr_1) != XCANFD_MODE_CONFIG);

	XCanFd_EnterMode(CanFdInstPtr_2, XCANFD_MODE_CONFIG);
	while (XCanFd_GetMode(CanFdInstPtr_2) != XCANFD_MODE_CONFIG);

	/*
	 * Configure the Baud Rate Prescalar in
	 * Arbitration Phase
	 */
	XCanFd_SetBaudRatePrescaler(CanFdInstPtr_1, TEST_BRPR_BAUD_PRESCALAR);
	XCanFd_SetBaudRatePrescaler(CanFdInstPtr_2, TEST_BRPR_BAUD_PRESCALAR);

	/*
	 * Configure the Bit Timing Values in
	 * Arbitration Phase.
	 */
	XCanFd_SetBitTiming(CanFdInstPtr_1, TEST_BTR_SYNCJUMPWIDTH,
		TEST_BTR_SECOND_TIMESEGMENT,TEST_BTR_FIRST_TIMESEGMENT);

	XCanFd_SetBitTiming(CanFdInstPtr_2, TEST_BTR_SYNCJUMPWIDTH,
		TEST_BTR_SECOND_TIMESEGMENT,TEST_BTR_FIRST_TIMESEGMENT);

	 /* Configure the Baud Rate Prescalar in Data Phase */
	XCanFd_SetFBaudRatePrescaler(CanFdInstPtr_1, TEST_FBRPR_BAUD_PRESCALAR);
	XCanFd_SetFBaudRatePrescaler(CanFdInstPtr_2, TEST_FBRPR_BAUD_PRESCALAR);

	/* Configure the Bit Timing Values in Data Phase */
	XCanFd_SetFBitTiming(CanFdInstPtr_1,TEST_FBTR_SYNCJUMPWIDTH,
		TEST_FBTR_SECOND_TIMESEGMENT,TEST_FBTR_FIRST_TIMESEGMENT);
	XCanFd_SetFBitTiming(CanFdInstPtr_2,TEST_FBTR_SYNCJUMPWIDTH,
			TEST_FBTR_SECOND_TIMESEGMENT,TEST_FBTR_FIRST_TIMESEGMENT);
	/*
	 ...
	 * Rate Switch or not.
	 */
	XCanFd_SetBitRateSwitch_DisableNominal(CanFdInstPtr_1);
	XCanFd_SetBitRateSwitch_DisableNominal(CanFdInstPtr_2);
	/*
	 * Check for the design
	 * 0 - Sequential Mode
	 * 1 - MailBox Mode.
	 */
	if (XCANFD_GET_RX_MODE(CanFdInstPtr_1) == 0) {
		XCanFd_AcceptFilterDisable(CanFdInstPtr_1,
			XCANFD_AFR_UAF_ALL_MASK);
		XCanFd_AcceptFilterEnable(CanFdInstPtr_1,
			XCANFD_AFR_UAF_ALL_MASK);
	}
	else {
		RxBuffers = XCanFd_Get_RxBuffers(CanFdInstPtr_1);
		IdValue = XCanFd_CreateIdValue(TEST_MESSAGE_ID, 0, 0, 0, 0);
		for (BuffNr = 0;BuffNr < RxBuffers;BuffNr++) {
			XCanFd_RxBuff_MailBox_DeActive(CanFdInstPtr_1,BuffNr);
			XCanFd_Set_MailBox_IdMask(CanFdInstPtr_1,BuffNr,
				TEST_MAILBOX_MASK,IdValue);
			XCanFd_RxBuff_MailBox_Active(CanFdInstPtr_1,BuffNr);
		}
	}

	if (XCANFD_GET_RX_MODE(CanFdInstPtr_2) == 0) {
			XCanFd_AcceptFilterDisable(CanFdInstPtr_2,
				XCANFD_AFR_UAF_ALL_MASK);
			XCanFd_AcceptFilterEnable(CanFdInstPtr_2,
				XCANFD_AFR_UAF_ALL_MASK);
		}
		else {
			RxBuffers = XCanFd_Get_RxBuffers(CanFdInstPtr_2);
			IdValue = XCanFd_CreateIdValue(TEST_MESSAGE_ID, 0, 0, 0, 0);
			for (BuffNr = 0;BuffNr < RxBuffers;BuffNr++) {
				XCanFd_RxBuff_MailBox_DeActive(CanFdInstPtr_2,BuffNr);
				XCanFd_Set_MailBox_IdMask(CanFdInstPtr_2,BuffNr,
					TEST_MAILBOX_MASK,IdValue);
				XCanFd_RxBuff_MailBox_Active(CanFdInstPtr_2,BuffNr);
			}
		}

	/* Configure CAN Device to enter to Loop Back Mode */
	XCanFd_EnterMode(CanFdInstPtr_1, XCANFD_MODE_NORMAL);
	while (XCanFd_GetMode(CanFdInstPtr_1) != XCANFD_MODE_NORMAL);

	XCanFd_EnterMode(CanFdInstPtr_2, XCANFD_MODE_NORMAL);
	while (XCanFd_GetMode(CanFdInstPtr_2) != XCANFD_MODE_NORMAL);

	/* Send Frame */
	Status = SendFrame(CanFdInstPtr_1);
	if (Status != XST_SUCCESS) {
		return Status;
	}

	/* Receive The Frame */
	Status = RecvFrame(CanFdInstPtr_2);
