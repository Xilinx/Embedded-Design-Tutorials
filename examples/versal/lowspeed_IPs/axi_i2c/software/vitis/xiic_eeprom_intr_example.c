...
int EepromReadData(u8 *BufferPtr, u16 ByteCount, u16 addr_location);
...
int main(void)
{
	...
	if (Status != XST_SUCCESS) {
		xil_printf("AXI IIC eeprom Example Failed\r\n");
		return XST_FAILURE;
	}
	xil_printf("Successfully ran AXI IIC eeprom Example\r\n");
	return XST_SUCCESS;
}
...
//	}
		for(int page_count = 0; page_count < 256; page_count++)
		{
			...
			WriteBuffer[0] = (u8) ((page_count * PAGE_SIZE) >> 8) & 0xFF;
			WriteBuffer[1] = (u8) ((page_count * PAGE_SIZE)) & 0xFF;

			for (Index = 0; Index < PAGE_SIZE; Index++) {
			WriteBuffer[2 + Index] = page_count;
			ReadBuffer[Index] = 0;
			}
			...
			* Write to the EEPROM.
			*/
			Status = EepromWriteData(2 + PAGE_SIZE);
			if (Status != XST_SUCCESS) {
			return XST_FAILURE;
			}
		}
	 ...
	 */
		for(int page_count = 0; page_count < 256; page_count++)
			...
			Status = EepromReadData(ReadBuffer, PAGE_SIZE, page_count * PAGE_SIZE);
			if (Status != XST_SUCCESS) {
				return XST_FAILURE;
			}
			 ...
			 */
			for (Index = 0; Index < PAGE_SIZE; Index++) {
				if (ReadBuffer[Index] != page_count) {
					return XST_FAILURE;
				}
				ReadBuffer[Index] = 0;
			}
...
int EepromReadData(u8 *BufferPtr, u16 ByteCount, u16 addr_location)
{
	...
	/*if (sizeof(Address) == 1) {
		...
		WriteBuffer[0] = (u8) (addr_location >> 8);
		WriteBuffer[1] = (u8) (addr_location);
	//}
